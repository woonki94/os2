...
boot_alloc(uint32_t n)
{
	...
	// cprintf("n: %u\nnextfree : %p\nPhysical memory of nextfree : %p\nnpages: %u\n",
	// n,nextfree,PADDR(nextfree), npages);
...
mem_init(void)
{
	...
	// Your code goes here:
    //boot_alloc(size) to allocate pages
    //total number = npages
    //size = size_t npages * sizeof(struct PageInfo)
    //use memset to initialize with 0
    //struct PagInfo * pages
	// cprintf("typeof size: %s\nNumber of pages:%u", npages * sizeof(struct PageInfo), npages );
...
page_init(void)
{
    ...
    // cprintf("PGNUM IO %u",PGNUM(IOPHYSMEM));

	size_t i;
	for (i = 0; i < npages; i++) {
        //if (i == 0 || ((PADDR(&pages[i]) > IOPHYSMEM)  && i < PGNUM(PADDR(boot_alloc(0))))) {
        if(i==0 || (i>= PGNUM(IOPHYSMEM) && i< PGNUM(PADDR(boot_alloc(0))))){
            // cprintf("PADDR(&pages[i]: %p\n%p", PADDR(&pages[i]),IOPHYSMEM);
            pages[i].pp_ref = 1;
        }
        else{
            pages[i].pp_ref = 0;        
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    //Invalid
        ...
        if(create){
            ...
            if (!pp_page_table) // Out of memory
                return NULL; 
...
{
	// Fill this function in
    //cprintf("pgdir_walk(pgdir,va,1): %p\n",  pgdir_walk(pgdir,va,1));
    //page2pa(pp);
...
{
	// Fill this function in
    pte_t *p_pte = pgdir_walk(pgdir, va, 0); //just look up. create flag = 0 

    if(pte_store!=NULL && p_pte){
        *pte_store = p_pte;
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    for(uint32_t i = start; i < end; i += PGSIZE){
        ...
        if (i >= ULIM || pte == NULL || !(*pte & PTE_P) || (*pte & perm) != perm) {
            user_mem_check_addr = (i < addr) ? addr : i;
            return -E_FAULT;
        }
    }
	return 0;
